...
// const fetchLib = fetch ? fetch : require( 'node-fetch' ) ;


export default function( path , options = {} ) {
	var basePath = options.basePath ? options.basePath : process.env.VUE_APP_API ;
	...
	if ( store.state.session && store.state.session.session && store.state.session.session.token ) {
	...
	return ( options.progress ? classicXHR : fetchLib )( `${basePath}${path}` , Object.assign( {} , requestOptions , options ) )
